...
% R = ones(N,1)*deg2rad(20);
R = zeros(N,1);

%P = (rand(N,1)*pi - pi/2);
P = (rand(N,1)*pi - pi/2)* 8/9;
% P = ones(N,1)*deg2rad(5);
%P = zeros(N,1);
...
Sm = eye(3,3)+ diag([ 1/10; 1/10; 1/10]);
bm = [1 ; 1; 1]* 1e-6;
sigma_m = 0.6 * 1e-6;
...
for i = 1:N
  ...
  %C_rpy_ned(1:3,1:3) = rpy2mat(RPY(i,1:3)');
  Quat_rpy_ned = rpy2q(RPY(i,1:3)');
  C_rpy_ned(1:3,1:3) = q2mat(Quat_rpy_ned);
  
  % формирование истинных значений Aned = 0;0;1 
  A_rpy(1:3,i) = C_rpy_ned(1:3,1:3)' *[0;
    0;
    1];
  M_rpy(1:3,i) = C_rpy_ned(1:3,1:3)' * M_ned;
  ...
  % для получения измерений акселерометров с погрешностями  
   ...
   % применение модели измерений магнитометров   для получения измерений магнитометров с погрешностями 
   ym_rpy(1:3,i) = m_meas(M_rpy(1:3,i), Sm, bm, sigma_m);
  
  % расчёт углов по измерениям акселерометров с погрешностями
  %RPY_new1 = angle_calc_ned(A_rpy(1:3,i));
  RPY_new1 = angle_calc(ya_rpy(1:3,i));
  RPY_new(i,1:3)= RPY_new1;
  
  % расчёт магнитного азимута по измерениям магнитометра
  %C_rpy_ned(1:3,1:3) = rpy2mat([R(i); P(i); 0]);
  %C_rpy_ned(1:3,1:3) = rpy2mat([RPY_new1(1:2)'; 0]);
  
  Quat_rpy_ned = rpy2q([RPY_new1(1:2)'; 0]);
  C_rpy_ned(1:3,1:3) = q2mat(Quat_rpy_ned);
  
  %ym_ned(1:3,i) = C_rpy_ned(1:3,1:3) * ym_rpy(1:3,i);
  ym_ned(1:3,i) = C_rpy_ned(1:3,1:3) * M_rpy(1:3,i);
  Azimuth_magn(i) = atan2(-ym_ned(2,i),ym_ned(1,i));
